...
public class FeedbackRestController {
    ...
    @GetMapping(path="/feedbacks/sentiment", produces = MediaType.IMAGE_PNG_VALUE)
    public BufferedImage getFeedbackCountBySentiment() {

        // Retrieve feedback data from database
        List<Feedback> feedbackList = retrieveAllFeedback();
        ...
        try {
            chartImage = chart.createBufferedImage(800, 600);
        } catch (Exception e) {
            e.printStackTrace();
        }
